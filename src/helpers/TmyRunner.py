...
def get_dynamic_threshold(current_epoch, alpha, max_threshold):
    return min(alpha * current_epoch, max_threshold)
def truncated_loss(loss_value, dynamic_threshold):
    # print(loss_value)
    loss_values = loss_value  # 逐元素损失值
    # print(loss_values,dynamic_threshold)
...
class TmyRunner(object):
	...
	def train(self, data_dict: Dict[str, BaseModel.Dataset]):
		...
		try:
			for epoch in range(self.epoch):
				dynamic_threshold = get_dynamic_threshold(epoch, alpha, max_threshold)
				# Fit
				self._check_time()
				gc.collect()
				torch.cuda.empty_cache()
				loss = self.fit(data_dict['train'], dynamic_threshold,epoch=epoch + 1)
				if np.isnan(loss):
	...
	def fit(self, dataset: BaseModel.Dataset,dynamic_threshold, epoch=-1,) -> float:
		model = dataset.model
		...
		for batch in tqdm(dl, leave=False, desc='Epoch {:<3}'.format(epoch), ncols=100, mininterval=1):
			...
			loss = truncated_loss(model.loss(out_dict),dynamic_threshold)
			loss.backward()
