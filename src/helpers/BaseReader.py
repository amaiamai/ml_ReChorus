...
class BaseReader(object):
    ...
    def _read_data(self):
        ...
        for key in ['train', 'dev', 'test']:
            ...
            file_path = os.path.join(os.path.dirname(current_dir), self.prefix, self.dataset, key + '.csv')
            self.data_df[key] = pd.read_csv(file_path, sep=self.sep).reset_index(drop=True).sort_values(by=['user_id', 'time'])
