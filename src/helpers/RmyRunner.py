...
def reweightedLoss(out_dict: dict) -> torch.Tensor:
        """
        BPR ranking loss with optimization on multiple negative samples, incorporating reweighted loss.
        ...
        # Calculate weights based on neg_pred using a power function, here beta is a hyperparameter
        beta = 0.05
        neg_weights = torch.pow(abs(neg_pred), beta)
        # Calculate weighted BPR loss
        pos_pred_expanded = pos_pred[:, None].expand_as(neg_pred)
        loss = -(((pos_pred_expanded - neg_pred).sigmoid() * neg_softmax * neg_weights).sum(dim=1)).clamp(min=1e-8, max=1-1e-8).log().mean()
        return loss

def reweighted_loss(loss_value,pre,beta):
    loss_values = loss_value  # 逐元素损失值
    weights = torch.pow(abs(pre), beta)  # 使用预测值作为权重基础
    weighted_loss = weights * loss_values
    # print(weights)
    return weighted_loss.mean()

class RmyRunner(object):
	@staticmethod
		...
		for k in topk:
			...
			for metric in metrics:
				...
				if metric == 'HR':
					...
					# Calculate Recall@K
					# Assuming there is only one positive item per row
					recall = hit.astype(int)
					evaluations[key] = recall.mean()
				else:
	...
	def train(self, data_dict: Dict[str, BaseModel.Dataset]):
		...
		try:
			for epoch in range(self.epoch):
				...
				loss = self.fit(data_dict['train'],beta,epoch=epoch + 1)
				if np.isnan(loss):
	...
	def fit(self, dataset: BaseModel.Dataset,beta, epoch=-1,) -> float:
		model = dataset.model
		...
		for batch in tqdm(dl, leave=False, desc='Epoch {:<3}'.format(epoch), ncols=100, mininterval=1):
			...
			# loss = reweighted_loss(model.loss(out_dict),out_dict['prediction'],beta)
			loss = reweightedLoss(out_dict)
			loss.backward()
